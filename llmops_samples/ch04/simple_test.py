...
from model_provider import ChatModelManager
...
def update_chain():
    if not st.session_state.get("chat_prompt_template"):
...
def new_prompt():
    ...
    st.text_input("저장할 프롬프트 이름을 입력하세요.", key="new_prompt_name")

def add_new_prompt():
    if PROMPT_HUB.add_prompt(
        st.session_state.get("new_prompt_name"),
...
def add_new_version():
    if PROMPT_HUB.add_prompt_version(
        ...
        st.session_state.get("system_prompt"), 
        st.session_state.get("user_prompt"),
        st.session_state.get("changed_details")):
...
def prompt():
    ...
    if st.session_state.get("is_exist_prompt"):
        ...
        if st.session_state.get("prompt_versions") is not None:
            st.dataframe(st.session_state.get("prompt_versions"), hide_index=True, width='stretch')
    else:
        new_prompt()
    ...
    system_prompt = st.session_state.get("system_prompt") or ""
    user_prompt = st.session_state.get("user_prompt") or ""
    ...
    if st.button("Run", on_click=update_chain, width='stretch'):
        with st.chat_message("ai"):
            st.write_stream(st.session_state.chain.stream(st.session_state.chain_input))
    
    if st.session_state.get("is_exist_prompt"):
        if st.button("Save New Version", width='stretch'):
            add_new_version()
    else:
        if st.button("Save New Prompt", width='stretch'):
            add_new_prompt()
